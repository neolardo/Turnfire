        ...
        if (property.isExpanded)
        {
            ...
            while (iterator.NextVisible(enterChildren) && !SerializedProperty.EqualContents(iterator, endProperty))
            {
                ...
                if (iterator.name == "_normalizedValue" || iterator.name == "normalizedValue")
                {
                    ...
                    var group = groupProp.objectReferenceValue as RangedStatIntGroupDefinition;
                    ...
                    int actualValue = Mathf.RoundToInt(Mathf.Lerp(group.Minimum, group.Maximum, valueProp.floatValue));

                    GUIContent decoratedLabel = new GUIContent($"Value (actual = {actualValue})");
